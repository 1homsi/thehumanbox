...
import type {
  ...
  HierarchyPointNode as D3HierarchyPointNode,
} from 'd3-hierarchy'
import {
  zoom as d3zoom,
  zoomIdentity,
  type ZoomTransform as D3ZoomTransform,
  type ZoomBehavior as D3ZoomBehavior,
} from 'd3-zoom'
...
export function FamilyTreeModal({ organisms: livOrgs, sexWords, onClose }: Props) {
  ...
  const draw = useCallback(() => {
    ...
    for (const { org, x, y } of nodes) {
      ...
      ctx.globalAlpha = isAlive ? 0.22 : 0.1
      ctx.fillStyle = color
    ...
    <Modal open onClose={onClose} className="tree-modal" title="Family tree" hideTitle>
      <div className="lang-modal-header">
        ...
        <span className="tree-modal-sub">
          {organisms.filter((o) => o.alive).length} alive · {organisms.filter((o) => !o.alive).length}{' '}
          ancestors · {(maxGen ?? 0) + 1} generations
        </span>
      ...
      <div className="tree-tooltip">
        {hovered ? (
          ...
          <span style={{ color: '#333' }}>
            scroll to zoom · drag to pan · hover a node for info · <span style={{ color: '#c97' }}>─ ─</span>{' '}
            bonded pair
          </span>
        )}
      </div>
      ...
      <div className="tree-legend">
        ...
        <span style={{ color: '#444', marginLeft: 8 }}>
          ...
          {sexWords && (
            <>
              {' '}
              · <span style={{ color: '#7ab0e0' }}>▪ {sexWords[0]}</span> ·{' '}
              <span style={{ color: '#e09ab0' }}>● {sexWords[1]}</span>
            </>
          )}
        </span>
