...
const GOV_EMOJI: Record<string, string> = {
  tribal: '\u{1F525}', chiefdom: '\u{1FAA9}', monarchy: '\u{1F451}',
  theocracy: '\u{271D}\u{FE0F}', republic: '\u{1F3DB}\u{FE0F}',
  democracy: '\u{1F5F3}\u{FE0F}', empire: '\u{1F985}',
  federation: '\u{1F30E}', corporate: '\u{1F3E2}',
}

const ART_EMOJI: Record<string, string> = {
  cave_painting: '\u{1F58C}\u{FE0F}', sculpture: '\u{1F5FF}',
  fresco: '\u{1F3DB}\u{FE0F}', painting: '\u{1F5BC}\u{FE0F}',
  photograph: '\u{1F4F7}', film: '\u{1F3AC}', digital: '\u{1F5A5}\u{FE0F}',
}
...
export function CivStatsModal({ world, onClose }: Props) {
  ...
  const artworks = (world as unknown as { artworks?: Array<{
    id: number; kind: string; title: string; creator_name: string
  }> }).artworks ?? []
      ...
      <div className="civ-modal-grid">
        <section className="civ-section">
          ...
          <table className="civ-table">
            <thead><tr><th>Name</th><th>Era</th><th>Pop</th><th>Gov</th><th>Currency</th></tr></thead>
            <tbody>
        ...
        <section className="civ-section">
          ...
          {governments.map(g => (
            <div key={g.lineage_id} className="civ-row">
              <span className="civ-row-head">{GOV_EMOJI[g.kind] ?? ''} {g.kind}</span>
              ...
              {g.laws && g.laws.length > 0 && (
                <div className="civ-row-laws">{g.laws.map(l => <span key={l} className="civ-chip">{l}</span>)}</div>
              )}
            </div>
          ))}
        </section>
        ...
        <section className="civ-section">
          ...
          {religions.map(r => (
            <div key={r.id} className="civ-row">
              ...
              <span className="civ-row-tag">founded by {lineageById(r.founder_lineage ?? r.lineage_id ?? '')}</span>
            </div>
          ))}
        </section>
        ...
        <section className="civ-section">
          ...
          <div className="civ-build-grid">
            {buildingRows.map(([kind, count]) => (
              <span key={kind} className="civ-chip">{kind}: {count}</span>
            ))}
          </div>
        </section>
        ...
        <section className="civ-section">
          ...
          {artworks.slice(0, 10).map(a => (
            <div key={a.id} className="civ-row">
              <span className="civ-row-head">{ART_EMOJI[a.kind] ?? ''} {a.title}</span>
