  ...
  return (
    <header className="header">
      ...
      <div className="header-badges">
        {world && (
          ...
          >
            <span className="daynight">
              <DayNightDial isDay={!!world.is_day} progress={world.day_progress ?? 0} />
        ...
        {fireTiles > 0 && (
          <Tooltip
            tip={`${fireTiles} tile${fireTiles > 1 ? 's' : ''} on fire - spreading to nearby flammable terrain`}
          >
        ...
        {sickOrgs > 0 && (
          <Tooltip
            tip={`${sickOrgs} organism${sickOrgs > 1 ? 's' : ''} infected - sickness spreads through close contact`}
          >
        ...
        <div className="header-actions">
          ...
          <Tooltip tip="Civilization: eras, governments, religions, buildings, books, art, and recent headlines">
            <button className="lang-btn" data-tour="civ-btn" onClick={openCiv}>
              <span className="btn-icon">{'\u{1F30D}'}</span>
          ...
          <div className="more-menu" ref={moreRef}>
            <button
              className={clsx('lang-btn', showMore && 'active')}
              data-tour="more-btn"
              onClick={toggleMore}
            >
              <span className="btn-icon">···</span>
      ...
      {world && (
        ...
        >
          <span className="btn-icon">{leftOpen ? '✕' : '≡'}</span>
      ...
      {world && (
        ...
        >
          <span className="btn-icon">{panelOpen ? '✕' : '≡'}</span>
        ...
        >
          {isFullscreen ? (
            <svg
              ...
              strokeWidth="1.5"
            >
              <path d="M4 1H1v3M8 1h3v3M4 11H1V8M8 11h3V8" />
              <path d="M4 4l-2 2 2 2M8 4l2 2-2 2" strokeWidth="1" />
            </svg>
          ) : (
            <svg
              width="12"
              ...
              strokeWidth="1.5"
            >
              <path d="M1 4V1h3M8 1h3v3M1 8v3h3M8 11h3V8" />
            </svg>
          )}
        </button>
      )}
