...
export function Fireflies({ hutPositions, isNight }: Props) {
  ...
  const params = useMemo(() => {
    ...
    return Array.from({ length: FIREFLY_COUNT }, (_, i) => {
      ...
      return {
        ...
        fz: 0.22 + (((s32 >>> 8) & 0xf) / 16) * 0.3,
        fy: 0.12 + (((s32 >>> 12) & 0xf) / 16) * 0.2,
  ...
  useFrame(({ clock }) => {
    ...
    for (let i = 0; i < params.length; i++) {
      ...
      const flicker = 0.45 + Math.sin(t * 5.2 + p.gp) * 0.32 + Math.sin(t * 13.7 + p.gp * 1.4) * 0.18
  ...
  return (
    <instancedMesh ref={meshRef} args={[FIREFLY_GEO, undefined, FIREFLY_COUNT]} frustumCulled={false}>
      <meshBasicMaterial color="#ccff30" transparent opacity={0.92} toneMapped={false} depthWrite={false} />
    </instancedMesh>
  )
}
