...
// hook owns, mutates the caches in place, and returns the new
// WorldState + the rebuilt GridState. The hook then plugs results into
// React state / interp refs.
//
// Kept separate from useSimulation.ts because this is the most
// bug-prone surface (delta merging, AoS/SoA branching, viewport vs
// full org lists) and where the next round of growth lands. Pulling
// it out also makes it directly unit-testable - feed in a frame +
// fake caches and assert the returned WorldState.

/** Merge a single field-wise: only copy keys whose value is neither
 *  null nor undefined. Used so partial deltas leave cached cold/warm
 *  fields untouched. */
...
export interface MergeResult {
  next: WorldState
  /** Whatever grid was built/merged for this frame - caller should
   *  assign it back into its cache ref so the next call sees it. */
  grid: GridState
}
 ...
 *
 * Mutates: `caches.organisms` and `caches.animals` are updated in
 * place (Map identity preserved on deltas; replaced on `*_complete`
 * frames). `caches.grid` is NOT mutated - caller must store
 * `result.grid` back into its ref.
 */
export function mergeFrame(parsed: IncomingWorldFrame, caches: MergeCaches): MergeResult {
  ...
  // Resolve organisms list from whichever wire format is in this frame.
  // Full snapshots ship AoS under `organisms`; deltas ship SoA under
  // `organisms_hot`, which we expand into the same partial-update shape
  // the AoS path used.
