import type { SceneOccupant } from '../types'
...
export function Occupant({ occupant, x, y, selected, onClick }: Props) {
  ...
  return (
    <g transform={`translate(${x} ${y}) scale(${scale})`} onClick={onClick} style={{ cursor: 'pointer' }}>
      ...
      {sex === 'male' && (
        <path d={`M -3 -6 Q 0 -7 3 -6 L 3 -4 L -3 -4 Z`} fill="#3a2618" opacity="0.7" />
      )}
      {org.is_elder && (
        <line x1="4" y1="2" x2="6" y2="9" stroke="#aa9070" strokeWidth="0.6" />
      )}
      {isChild && (
        <circle cx="0" cy="0" r="0" fill="none" />
      )}
      {role === 'host' && (
        <text x="0" y="-10" textAnchor="middle" fontSize="3.2" fill="#ffe066"
              style={{ fontWeight: 700, letterSpacing: 0.2, pointerEvents: 'none' }}>★</text>
      )}
      <text x="0" y="14" textAnchor="middle" fontSize="3.5" fill="#e5ddc7"
            style={{ pointerEvents: 'none', fontFamily: 'monospace' }}>
        {org.name}
      </text>
      {activity && (
        <text x="0" y="19" textAnchor="middle" fontSize="2.6" fill="#9b9180"
