...
export function OrgLabels({ organisms, depthMap, biomes }: Props) {
  ...
  const labels = useMemo(() => {
    ...
    const scored: { id: string; name: string; pos: [number, number, number]; d: number }[] = []
  ...
  }, [organisms, depthMap, biomes, camera.position.x, camera.position.z])
      ...
      {labels.map(l => (
        <Billboard key={l.id} position={l.pos} frustumCulled={false}>
          <Text
            ...
            frustumCulled={false}
            renderOrder={999}
            // Match the lineage color when there is one.
