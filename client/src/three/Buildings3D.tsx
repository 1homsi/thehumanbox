...
const LIB_ROOF       = (() => { const g = new ConeGeometry(4.0, 1.8, 4); g.rotateY(Math.PI / 4); return g })()
const LIB_BOOK       = new BoxGeometry(0.5, 0.9, 0.32)
const MARKET_ROOF    = new BoxGeometry(5.6, 0.28, 4.4)
const MARKET_POST    = new CylinderGeometry(0.18, 0.20, 3.2, 6)
...
const MILL_ROOF      = (() => { const g = new ConeGeometry(3.0, 1.6, 4); g.rotateY(Math.PI / 4); return g })()
...
const LIGHTHOUSE_CAP = (() => { const g = new ConeGeometry(0.9, 1.0, 8); return g })()
const TOWER_COL      = new CylinderGeometry(1.2, 1.4, 9.0, 8)
const TOWER_CAP      = (() => { const g = new ConeGeometry(1.55, 2.0, 8); return g })()
...
function getMat(color: string, opts?: { emissive?: string; emissiveIntensity?: number }): MeshStandardMaterial {
...
function Layer({
  positions, yOffset, geometry, color, emissive, emissiveIntensity,
  maxCount, scale = 1, rotY = 0,
}: LayerProps) {
...
function SpinningBlades({
  positions, axis,
}: { positions: [number, number, number][]; axis: 'z' | 'x' }) {
  const meshRef = useRef<InstancedMesh>(null)
...
function LighthouseBeams({ positions }: { positions: [number, number, number][] }) {
  const lights = useMemo(() => positions.map(() => {
...
function groupBuildings(
  buildings: Building[], depthMap: number[][], biomes: number[][],
...
export function Buildings3D({ buildings, depthMap, biomes }: Props) {
  ...
  return (
    <>
      ...
      <Layer
        positions={apartments} yOffset={7.0}
        geometry={APARTMENT_GEO} color="#a0a0a0"
        maxCount={cap(apartments.length)}
      />

      <Layer
        positions={schools} yOffset={1.8}
        geometry={SCHOOL_MID} color={FN_COLOR.Education}
        maxCount={cap(schools.length)}
      />
      <Layer
        positions={schools.map(([x, y, z]) => [x - 2.0, y, z - 1.5] as [number, number, number])}
        yOffset={1.8}
        geometry={SCHOOL_WING} color={FN_COLOR.Education}
        maxCount={cap(schools.length)}
      />
      <Layer
        positions={schools.map(([x, y, z]) => [x + 2.0, y, z - 1.5] as [number, number, number])}
      ...
      <Layer
        positions={universities} yOffset={3.5}
        geometry={UNI_BLOCK} color={FN_COLOR.Education}
        maxCount={cap(universities.length)}
      />
      <Layer
        positions={universities.map(([x, y, z]) => [x - 2.4, y, z + 2.8] as [number, number, number])}
        yOffset={2.8} geometry={UNI_COL} color="#e8dcb6"
        maxCount={cap(universities.length)}
      />
      <Layer
      ...
      <Layer
        positions={libraries} yOffset={1.8}
        geometry={LIB_BASE} color={FN_COLOR.Education}
        maxCount={cap(libraries.length)}
      />
      <Layer
        positions={libraries} yOffset={4.5}
        geometry={LIB_ROOF} color={ROOF_DARK}
        maxCount={cap(libraries.length)}
      />
      <Layer
        positions={libraries.map(([x, y, z]) => [x - 1.6, y, z + 3.05] as [number, number, number])}
        yOffset={1.0} geometry={LIB_BOOK} color="#8a3030"
        maxCount={cap(libraries.length)}
      />
      <Layer
        positions={libraries.map(([x, y, z]) => [x - 0.6, y, z + 3.05] as [number, number, number])}
        yOffset={1.0} geometry={LIB_BOOK} color="#306030"
        maxCount={cap(libraries.length)}
      />
      <Layer
        positions={libraries.map(([x, y, z]) => [x + 0.6, y, z + 3.05] as [number, number, number])}
        yOffset={1.0} geometry={LIB_BOOK} color="#304878"
        maxCount={cap(libraries.length)}
      />
      <Layer
        positions={libraries.map(([x, y, z]) => [x + 1.6, y, z + 3.05] as [number, number, number])}
        yOffset={1.0} geometry={LIB_BOOK} color="#a07030"
        maxCount={cap(libraries.length)}
      />

      <Layer
        positions={markets} yOffset={3.3}
        geometry={MARKET_ROOF} color={FN_COLOR.Commerce}
        maxCount={cap(markets.length)}
      />
      <Layer
        positions={markets.map(([x, y, z]) => [x - 2.4, y, z - 1.8] as [number, number, number])}
        yOffset={1.6} geometry={MARKET_POST} color="#5a3a1a"
        maxCount={cap(markets.length)}
      />
      <Layer
        positions={markets.map(([x, y, z]) => [x + 2.4, y, z - 1.8] as [number, number, number])}
      ...
      <Layer
        positions={temples} yOffset={1.1}
        geometry={TEMPLE_T1} color={FN_COLOR.Worship}
        maxCount={cap(temples.length)}
      />
      <Layer
        positions={temples} yOffset={3.2}
        geometry={TEMPLE_T2} color={FN_COLOR.Worship}
        maxCount={cap(temples.length)}
      />
      <Layer
        positions={temples} yOffset={5.1}
        geometry={TEMPLE_T3} color={FN_COLOR.Worship}
        maxCount={cap(temples.length)}
      />
      <Layer
        positions={temples} yOffset={6.8}
        geometry={TEMPLE_T4} color="#e8c870"
        maxCount={cap(temples.length)}
      />

      <Layer
        positions={factories} yOffset={2.2}
        geometry={FACTORY_BLOCK} color={FN_COLOR.Industry}
        maxCount={cap(factories.length)}
      />
      <Layer
        positions={factories.map(([x, y, z]) => [x - 2.2, y, z - 1.6] as [number, number, number])}
        yOffset={7.2} geometry={FACTORY_STACK} color="#3a3a3a"
        maxCount={cap(factories.length)}
      />
      <Layer
        positions={factories.map(([x, y, z]) => [x + 2.2, y, z - 1.6] as [number, number, number])}
      ...
      <Layer
        positions={hospitals} yOffset={2.2}
        geometry={HOSP_BLOCK} color={FN_COLOR.Healthcare}
        maxCount={cap(hospitals.length)}
      />
      <Layer
        positions={hospitals.map(([x, y, z]) => [x, y, z + 2.55] as [number, number, number])}
        yOffset={2.6} geometry={HOSP_CROSS_V}
        color="#ffffff" emissive="#d83030" emissiveIntensity={1.4}
        maxCount={cap(hospitals.length)}
      />
      <Layer
        positions={hospitals.map(([x, y, z]) => [x, y, z + 2.56] as [number, number, number])}
        yOffset={2.6} geometry={HOSP_CROSS_H}
        color="#ffffff" emissive="#d83030" emissiveIntensity={1.4}
        maxCount={cap(hospitals.length)}
      />

      <Layer
        positions={forges} yOffset={1.3}
        geometry={FORGE_BLOCK} color="#5a4030"
        maxCount={cap(forges.length)}
      />
      <Layer
        positions={forges.map(([x, y, z]) => [x, y, z + 2.05] as [number, number, number])}
        yOffset={1.6} geometry={FORGE_WIN}
        color="#ff8030" emissive="#ff7020" emissiveIntensity={2.2}
        maxCount={cap(forges.length)}
      />
      <ForgeGlows positions={forges} />

      <Layer
        positions={mills} yOffset={1.7}
        geometry={MILL_BASE} color="#a07854"
        maxCount={cap(mills.length)}
      />
      <Layer
        positions={mills} yOffset={4.1}
        geometry={MILL_ROOF} color={ROOF_DARK}
        maxCount={cap(mills.length)}
      />

      <Layer
        positions={windmills} yOffset={1.7}
        geometry={MILL_BASE} color="#a07854"
        maxCount={cap(windmills.length)}
      />
      <Layer
        positions={windmills} yOffset={4.1}
        geometry={MILL_ROOF} color={ROOF_DARK}
        maxCount={cap(windmills.length)}
      />
      <Layer
        positions={windmills.map(([x, y, z]) => [x, y, z + 1.9] as [number, number, number])}
        yOffset={4.2} geometry={BLADE_HUB} color="#3a2a1a"
        maxCount={cap(windmills.length)} rotY={Math.PI / 2}
      />
      <SpinningBlades
      ...
      <Layer
        positions={watermills} yOffset={1.7}
        geometry={MILL_BASE} color="#a07854"
        maxCount={cap(watermills.length)}
      />
      <Layer
        positions={watermills} yOffset={4.1}
        geometry={MILL_ROOF} color={ROOF_DARK}
        maxCount={cap(watermills.length)}
      />
      <SpinningBlades
      ...
      <Layer
        positions={lighthouses} yOffset={4.25}
        geometry={LIGHTHOUSE_COL} color="#e8e0d0"
        maxCount={cap(lighthouses.length)}
      />
      <Layer
        positions={lighthouses} yOffset={9.3}
        geometry={LIGHTHOUSE_TOP}
        color="#ffe8a0" emissive="#ffd060" emissiveIntensity={1.4}
        maxCount={cap(lighthouses.length)}
      />
      <Layer
        positions={lighthouses} yOffset={10.6}
        geometry={LIGHTHOUSE_CAP} color={ROOF_DARK}
        maxCount={cap(lighthouses.length)}
      />
      <LighthouseBeams positions={lighthouses} />

      <Layer
        positions={towers} yOffset={4.5}
        geometry={TOWER_COL} color={FN_COLOR.Military}
        maxCount={cap(towers.length)}
      />
      <Layer
        positions={towers} yOffset={10.0}
        geometry={TOWER_CAP} color={ROOF_DARK}
        maxCount={cap(towers.length)}
      />

      <Layer
        positions={bridges} yOffset={0.4}
        geometry={BRIDGE_GEO} color={FN_COLOR.Infrastructure}
        maxCount={cap(bridges.length)}
      />

      <Layer
        positions={walls} yOffset={1.2}
        geometry={WALL_GEO} color="#7a7268"
        maxCount={cap(walls.length)}
      />

      <Layer
        positions={aqueducts} yOffset={3.6}
        geometry={AQUE_TOP} color={FN_COLOR.Infrastructure}
        maxCount={cap(aqueducts.length)}
      />
      <Layer
        positions={aqueducts.map(([x, y, z]) => [x - 3.0, y, z] as [number, number, number])}
        yOffset={1.5} geometry={AQUE_LEG} color={FN_COLOR.Infrastructure}
        maxCount={cap(aqueducts.length)}
      />
      <Layer
