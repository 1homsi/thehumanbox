...
impl Simulation {
    pub(crate) fn spawn_founders(&mut self) {
        ...
        let mut sector_order: Vec<(i32, i32)> =
        ...
        if anchors.len() < N_TRIBES {
            ...
            while placed < N_TRIBES && i < n {
                ...
                all_land.swap(i, j);
                let (x, y) = all_land[i];
                let far_enough = anchors
                    .iter()
                    .all(|&(ax, ay)| (ax - x).abs() + (ay - y).abs() >= 40);
                if far_enough {
                    anchors.push((x, y));
                    placed += 1;
                }
    ...
    pub(crate) fn spawn_immigrant_tribe(&mut self) {
                ...
                'wn: for dx in -4i32..=4 {
                    for dy in -4i32..=4 {
                        if WorldGrid::in_bounds(x + dx, y + dy)
                            && matches!(self.grid.get(x + dx, y + dy), Tile::Water)
                        {
                            water_near = true;
                            break 'wn;
                        }
        ...
        // densest cluster.
        let scored: Vec<(i32, i32, f32)> = candidates
            .into_iter()
            .map(|(x, y, fert)| {
                let nearest_org_d2: f32 = self
                    .organisms
                    .iter()
                    .filter(|o| o.alive)
        ...
        for k in 0..take {
            ...
            let max_age = self.rng.random_range(
                (8000.0 + 4000.0 * traits.resilience) as u32..=(18000.0 + 8000.0 * traits.resilience) as u32,
            );
            let mut org = Organism::new(
                id.clone(),
                generate_name(&mut self.rng, sex),
                lx as f32,
                ly as f32,
                0,
                String::new(),
    ...
    pub(crate) fn find_far_empty_anchor(&mut self, fx: i32, fy: i32) -> Option<(i32, i32)> {
        ...
        for _ in 0..120 {
            ...
            if (tx - fx).abs() + (ty - fy).abs() < MIN_DIST_FROM_FORKER {
                continue;
            }
            ...
            let too_close = self.organisms.iter().any(|o| {
                o.alive
                    && (o.home_x as i32 - tx).abs() + (o.home_y as i32 - ty).abs() < MIN_DIST_FROM_OTHER_HOMES
            });
            if too_close {
                continue;
            }
