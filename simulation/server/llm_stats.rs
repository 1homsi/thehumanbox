...
impl LlmStats {
    ...
    pub fn note_think_429(&self) {
        self.think_429.fetch_add(1, Ordering::Relaxed);
    }
    pub fn note_think_5xx(&self) {
        self.think_5xx.fetch_add(1, Ordering::Relaxed);
    }
    pub fn note_think_local_fallback(&self) {
        self.think_local_fallback.fetch_add(1, Ordering::Relaxed);
    }
    pub fn note_narration_429(&self) {
        self.narration_429.fetch_add(1, Ordering::Relaxed);
    }
    pub fn note_narration_5xx(&self) {
        self.narration_5xx.fetch_add(1, Ordering::Relaxed);
    }
    pub fn note_conversation_429(&self) {
        self.conversation_429.fetch_add(1, Ordering::Relaxed);
    }
    pub fn note_conversation_5xx(&self) {
        self.conversation_5xx.fetch_add(1, Ordering::Relaxed);
    }

    pub fn snapshot(&self) -> LlmStatsSnapshot {
        let (n_avg, n_p95) = self
            .narration_ms
            .lock()
            .map(|w| (w.avg(), w.p95()))
            .unwrap_or((0, 0));
        let (t_avg, t_p95) = self.think_ms.lock().map(|w| (w.avg(), w.p95())).unwrap_or((0, 0));
        let (c_avg, c_p95) = self
            .conversation_ms
            .lock()
            .map(|w| (w.avg(), w.p95()))
            .unwrap_or((0, 0));
