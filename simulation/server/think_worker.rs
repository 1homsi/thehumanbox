...
use crate::server::llm::{
    llm_body_with_temp_stop, llm_extract, strip_thinking, GroqResponse, THINK_LLM_MODEL, THINK_LLM_URL,
};
...
pub fn build_result_from_local(
    trigger: &ThinkTrigger,
    local: local_think::LocalResult,
) -> Option<ThinkResult> {
    let result = match trigger.scenario.as_str() {
        ...
        "council" => ThinkResult {
            ...
            thought: Some(format!(
                "the tribe should {}",
                local.strategy.unwrap_or(local.word)
            )),
...
pub fn deterministic_think_seed(trigger: &ThinkTrigger, attempt: u8) -> u64 {
    ...
    for d in &trigger.discoveries {
        h = mix_str(h, d);
    }
    for l in &trigger.life_log_top {
        h = mix_str(h, l);
    }
    h = mix_str(h, &trigger.emotional_state);
    h = mix_str(h, trigger.other_name.as_deref().unwrap_or(""));
    for d in &trigger.other_discoveries {
        h = mix_str(h, d);
    }
...
fn personality_summary(t: &ThinkTrigger) -> String {
    ...
    } else if t.aggression < 0.3 {
        parts.push("peaceful");
    }
    if t.curiosity > 0.7 {
        parts.push("curious");
    } else if t.curiosity < 0.3 {
        parts.push("incurious");
    }
    if t.social_tendency > 0.7 {
        parts.push("very social");
    } else if t.social_tendency < 0.3 {
        parts.push("solitary");
    }
    if t.fear > 0.7 {
        parts.push("fearful");
    ...
    if t.resilience > 0.7 {
        parts.push("resilient");
    } else if t.resilience < 0.3 {
        parts.push("fragile");
    }
    if parts.is_empty() {
        "balanced".to_string()
...
fn build_prompt(trigger: &ThinkTrigger) -> (String, u32) {
    ...
    let emotion = if trigger.emotional_state.is_empty() {
        "calm".to_string()
    } else {
        trigger.emotional_state.clone()
    };
    let knowledge = if trigger.discoveries.is_empty() {
        "nothing yet".to_string()
    } else {
        trigger.discoveries.join(", ")
    };
    let memories = if trigger.life_log_top.is_empty() {
    ...
    let stable_header = "You are a primitive creature surviving in a harsh world.\n\
    ...
    let world_ctx = {
        ...
        if !trigger.world_era.is_empty() {
            parts.push(format!("Era: {}", trigger.world_era));
        }
        if !trigger.season.is_empty() {
            parts.push(format!("Season: {}", trigger.season));
        }
        if parts.is_empty() {
            String::new()
        } else {
    ...
    let max_tokens = if trigger.scenario == "elder_teaching" {
        30
    } else {
        60
    };
...
fn build_result_from_llm(trigger: &ThinkTrigger, response: &str) -> Option<ThinkResult> {
    ...
    if trigger.scenario == "elder_teaching" {
        ...
        let teaching_raw = extract_tagged(response, "TEACHING:").unwrap_or_else(|| strip_thinking(response));
    ...
    let thought = extract_tagged(response, "THOUGHT:").unwrap_or_else(|| {
    ...
    let action_raw = extract_tagged(response, "ACTION:").unwrap_or_default();
    ...
    match trigger.scenario.as_str() {
        "first_contact" => {
            let delta: f32 = if action.starts_with("friend") {
                0.35
            } else if action.starts_with("hostil") || action.starts_with("attack") {
                -0.4
            } else {
                0.0
            };
            Some(ThinkResult {
                ...
                thought: Some(if thought.is_empty() {
                    "watching the stranger".to_string()
                } else {
                    thought
                }),
                ..Default::default()
        ...
        "council" => {
            let strategy = if action.starts_with("settle") || action.starts_with("build") {
                "settle"
            } else if action.starts_with("hunt") || action.starts_with("food") {
                "hunt"
            } else {
                "explore"
            };
            Some(ThinkResult {
                org_id: trigger.org_id.clone(),
                thought: Some(if thought.is_empty() {
                    format!("the tribe should {}", strategy)
                } else {
                    thought
                }),
                strategy_lineage: Some(trigger.lineage_id.clone()),
        ...
        "survival_crisis" => {
            let directive =
                if action.starts_with("food") || action.starts_with("eat") || action.starts_with("hunt") {
                    "seek_food"
                } else if action.starts_with("water") || action.starts_with("drink") {
                    "seek_water"
                } else {
                    "flee"
                };
            Some(ThinkResult {
                org_id: trigger.org_id.clone(),
                thought: Some(if thought.is_empty() {
                    format!("desperate for {}", directive.replace("seek_", ""))
                } else {
                    thought
                }),
        ...
        "abundance" => {
            let directive = if action.starts_with("social")
                || action.starts_with("gather")
                || action.starts_with("celebrat")
            {
                "socialize"
            } else if action.starts_with("explor") || action.starts_with("wander") {
                "explore"
            } else {
                "socialize"
            };
        ...
        "threat" => {
            let directive = if action.starts_with("fight")
                || action.starts_with("attack")
                || action.starts_with("defend")
            {
                "fight"
            } else if action.starts_with("trade") || action.starts_with("peace") || action.starts_with("gift")
            {
                "trade"
            } else {
                "flee"
        ...
        "lonely" => {
            let directive = if action.starts_with("seek_kin")
                || action.starts_with("family")
                || action.starts_with("kin")
            {
                "socialize"
            } else if action.starts_with("find_stranger") || action.starts_with("stranger") {
                "trade"
            } else {
                "explore"
            };
        ...
        "restless" => {
            let directive = if action.starts_with("build") {
                "explore"
        ...
        "invention" => {
            ...
            Some(ThinkResult {
                ...
                thought: Some(if thought.is_empty() {
                    format!("eureka: {}", discovery.replace('_', " "))
                } else {
                    thought
                }),
                ..Default::default()
        ...
        "reflection" => {
            let (trait_name, delta): (&str, f32) =
                if resp_lower.contains("brave") || resp_lower.contains("courag") {
                    ("fear", -0.06)
                } else if resp_lower.contains("social")
                    || resp_lower.contains("kind")
                    || resp_lower.contains("friend")
                {
                    ("social_tendency", 0.05)
                } else if resp_lower.contains("curious")
                    || resp_lower.contains("wonder")
                    || resp_lower.contains("explor")
                {
                    ("curiosity", 0.05)
                } else if resp_lower.contains("aggress")
                    || resp_lower.contains("fierce")
                    || resp_lower.contains("strong")
                {
                    ("aggression", 0.04)
                } else {
                    ("resilience", 0.04)
        ...
        "negotiation" => {
            let alliance = if resp_lower.contains("food")
                || resp_lower.contains("share")
                || resp_lower.contains("feast")
            {
                "food_sharing"
            } else if resp_lower.contains("defense")
                || resp_lower.contains("defend")
                || resp_lower.contains("protect")
                || resp_lower.contains("pact")
            {
                "defense_pact"
            } else if resp_lower.contains("knowledge")
                || resp_lower.contains("teach")
                || resp_lower.contains("learn")
            {
                "knowledge_exchange"
            } else {
                "territory"
        ...
        "grief" => Some(ThinkResult {
            org_id: trigger.org_id.clone(),
            thought: Some(if thought.is_empty() {
                "lost someone close".to_string()
            } else {
                thought
            }),
            ..Default::default()
        ...
        "illness" => {
            let directive = if action.starts_with("rest") {
                "rest"
            } else if action.starts_with("isolat") {
                "isolate"
            } else {
                "seek_help"
            };
        ...
        "migration" => {
            let directive = if action.starts_with("migrat") || action.starts_with("move") {
                "explore"
            } else if action.starts_with("forage") || action.starts_with("hunt") {
                "seek_food"
            } else {
                "rest"
            };
...
pub async fn think_worker(
    ...
    let mut retry_queue: std::collections::VecDeque<(ThinkTrigger, u8)> = std::collections::VecDeque::new();
    ...
    loop {
        ...
        if let Some(ref l) = limiter {
            ...
            match tokio::time::timeout(std::time::Duration::from_secs(5), l.acquire()).await {
                Ok(()) => {}
                Err(_) => {
                    tracing::warn!(
                        target: "think",
...
mod tests {
    ...
    fn extract_tagged_finds_thought_and_action() {
        ...
        assert_eq!(
            extract_tagged(resp, "THOUGHT:").as_deref(),
            Some("I feel hungry and must find food soon.")
        );
