...
fn tile_color(t: Tile, biome: u8, day_t: f32) -> [u8; 3] {
    ...
    let dim = if matches!(t, Tile::Fire | Tile::Campfire) {
        1.0
    } else {
        bright
    };
        ...
        for dy in -2i32..=2 {
            for dx in -2i32..=2 {
                ...
                img.put_pixel(
                    px as u32,
                    py as u32,
                    Rgba([a.color[0], a.color[1], a.color[2], 255]),
                );
            }
        }
    }
        ...
        for dy in -3i32..=3 {
            for dx in -3i32..=3 {
                ...
                img.put_pixel(
                    px as u32,
                    py as u32,
                    Rgba([o.color[0], o.color[1], o.color[2], 255]),
                );
            }
        }
    }
