...
pub fn apply(ctx: &mut ActionCtx) -> f32 {
    ...
    let pick = ctx.kin.iter().copied()
        .min_by(|&a, &b| {
            ctx.sim.organisms[a].health.partial_cmp(&ctx.sim.organisms[b].health)
                .unwrap_or(std::cmp::Ordering::Equal)
        });
    let Some(ki) = pick else { return 0.0; };
    {
    ...
    ctx.event("defense", "stood guard to protect the most vulnerable family member");
    0.007
}
